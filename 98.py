...
def FindObjects_Step1(img):
    ...
    for y in range(0,H,4):
        for x in range(0,W,4):
            ...
            get_hog.append(hog1)
            get_coordinates.append([max(0,x-21),max(0,y-21),min(W+1,x+22),min(H+1,y+22),0])
            #矩形56*56
            ...
            get_hog.append(hog2)
            get_coordinates.append([max(0,x-28),max(0,y-28),min(W+1,x+29),min(H+1,y+29),0])
            #矩形70*70
            ...
            get_hog.append(hog3)
            get_coordinates.append([max(0,x-35),max(0,y-35),min(W+1,x+36),min(H+1,y+36),0])
...
for i in range(len(imori_face)):
    cv2.rectangle(img2,(imori_face[i,0],imori_face[i,1]),(imori_face[i,2],imori_face[i,3]),(0,0,255))
img2=img2.astype(np.uint8)
cv2.imwrite("./output_image/output98.jpg",img2)
