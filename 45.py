...
def Hough_step_2(table):
    H,W=table.shape
    for y in range(H):
        for x in range(W):
            if np.max(table[max(0,y-1):min(H-1,y+2),max(0,x-1):min(W-1,x+2)])!=table[y,x]:
                table[y,x]=0
...
table=Hough_step_1(edge_hist)
#cv2.imwrite("./output_image/output42_1.jpg",angle)
hough=Hough_step_2(table)
