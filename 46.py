...
def Hough_step_3(img,table):
    ...
    index_y=index_x.copy()
    te=index_x%180
    #r/2 : Hough変換するときにずらした分
    rh=index_y//180-r/2
    #逆変換
    for te,rh in zip(te,rh): #zip : 短い方に合わせる
        t=te*np.pi/180.
        for y in range(H):
            if np.cos(t)!=0:
                x=int(-y*np.sin(t)/np.cos(t)+rh/np.cos(t))
                if 0<=x and x<W:
                    out[y,x]=[0,0,255] #条件を満たす点を赤色にする。
        for x in range(W):
            if np.sin(t)!=0:
                y=int(-x*np.cos(t)/np.sin(t)+rh/np.sin(t))
                if 0<=y and y<H:
                    out[y,x]=[0,0,255]
    out=out.astype(np.uint8)
    return out
...
#inverse_Hough
in_hou=Hough_step_3(img,out)

cv2.imwrite("./output_image/output46.jpg", in_hou)
cv2.imshow("result", in_hou)
cv2.waitKey(0)
