...
def SSD_matching(img,part):
    ...
    for y in range(H-h):
        for x in range(W-w):
            S=np.sum((img[y:y+h,x:x+w]-part)**2)
            if S<S_min:
                S_min=S
                x_left=x
                x_right=x+w
...
x_left,x_right,y_up,y_bottom=SSD_matching(img,part)
cv2.rectangle(img_ori,(x_left,y_up),(x_right,y_bottom),(0,0,255))
cv2.imshow("result",img_ori)
cv2.imwrite("./output_image/output54.jpg",img_ori)
cv2.waitKey(0)
